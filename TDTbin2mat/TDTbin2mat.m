...
data.info.blockname = tsqList(1).name(1:end-3);

data.info.date = datestr(datenum([1970, 1, 1, 0, 0, start_time]),'yyyy-mmm-dd');
data.info.starttime = datestr(datenum([1970, 1, 1, 0, 0, start_time]),'HH:MM:SS');
data.info.stoptime = datestr(datenum([1970, 1, 1, 0, 0, stop_time]),'HH:MM:SS');

s1 = datenum([1970, 1, 1, 0, 0, start_time]);
s2 = datenum([1970, 1, 1, 0, 0, stop_time]);

if stop_time > 0
   data.info.duration = datestr(s2-s1,'HH:MM:SS');
end

data.info.blocktime = datestr(datenum([1970, 1, 1, 0, 0, start_time]));
...
for i = 1:numel(unique_codes)
    ...
    if isequal(currentType, 'epocs')
        if strcmp(currentName(end), '\') || strcmp(currentName(end), '/')
            % this is an offset epoc for another epoc event.  find it.
            % CE: not supposed to see 'xx/' but my data happened to contain both 'Ep1/' and 'Ep1\' for some reasons
            % so I added the || statement. perhaps this should be removed eventually
            bFound = 0;
            if ~isempty(data.epocs)
                for x = fields(data.epocs)
                    field = x{1};
                    if strcmp(currentName(1:3), field(1:3)) == 1
                        bFound = 1;
                        data.epocs.(field).offset = timestamps(ind)';
                    end
                end
            end
            if bFound == 0
                warning(['onset epoc not found for ' currentName])
            end
        else
...
%             end
    ...
    elseif isequal(currentType, 'snips')
        ...
        data.snips.(currentName).sortname = 'TankSort';
        data.snips.(currentName).fs = freqs(ind(1)); 
%         data.snips.(currentName).offsets = all_offsets;
%         data.snips.(currentName).offset = ;% TODO add others
    ...
    elseif isequal(currentType, 'streams')
        ...
        if CHANNEL == 0
            ...
            data.streams.(currentName).data = zeros(1, npts*numel(ind)/nchan, fmt);
        end
        ...
        if useSEVs
            ...
            for f = 1:numel(ind)
                ...
                npts = (all_sizes(f)-10) * 4/sz;
                if CHANNEL > 0
                    data.streams.(currentName).data(1,start:start+npts-1) = fread(tev, npts, ['*' fmt])';
                else
                    data.streams.(currentName).data(1,start:start+npts-1) = fread(tev, npts, ['*' fmt])';
                end

...
num_chan    = size(LC_data.titles,1);
...
for c = 1:num_chan
    ...
    chan_data{c}    = LC_data.data(LC_data.datastart(c):LC_data.dataend(c));
    % find channel types
...
if isempty(cx_stim_chan)
    ...
    stim_bin = debounce(stim_bin,0.2*LC_data.samplerate(cx_stim_chan));
    num_stim = length(stim_bin);
    
    % convert to time
    ELF_data.epocs.Stim.onset = stim_bin'/LC_data.samplerate(cx_stim_chan);
end
